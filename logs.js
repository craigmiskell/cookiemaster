...
async function displayLogs() {
  ...
  if(logs.length == 0) {
    //No logs (maybe a clear has just been requested?)  Just clear out and get out, avoiding complicated
    // code later
...
async function clear(e) {
  let saveUpdating = updating;
  await browser.runtime.sendMessage({type: MessageTypes.ClearLogs});
  displayLogs();
